import Link from "next/link"

export function ScoutLogo() {
  return (
    <Link href="/" className="flex items-center space-x-2">
      <div className="flex items-center">
        <svg width="40" height="40" viewBox="0 0 290 290" fill="none" xmlns="http://www.w3.org/2000/svg">
          <rect width="290" height="290" rx="145" fill="#0066CC" />
          <g clipPath="url(#clip0_8_119)">
            <path
              d="M170.957 170.612C173.71 172.419 176.062 174.787 178.793 176.633C191.376 185.141 206.379 187.927 221.327 185.511C224.377 185.019 227.23 183.704 230.154 183.533C223.405 190.552 217.387 198.365 210.661 205.39C206.781 209.444 203.333 213.066 198.051 215.337C179.664 223.24 160.387 212.75 155.578 193.846C164.543 189.014 170.426 181.05 170.954 170.612L170.957 170.612Z"
              fill="white"
            />
            <path
              d="M60.9971 183.305C81.8928 190.385 103.946 185.087 120.197 170.612C120.278 180.342 126.113 189.654 135.181 193.225C130.726 216.43 102.248 226.27 84.4822 209.852L60.9939 183.301L60.9971 183.305Z"
              fill="white"
            />
            <path
              d="M145.442 150.726C134.429 150.726 125.502 159.654 125.502 170.666C125.502 181.679 134.429 190.607 145.442 190.607C156.455 190.607 165.382 181.679 165.382 170.666C165.382 159.654 156.455 150.726 145.442 150.726Z"
              fill="white"
            />
            <path
              d="M24.8125 126.137C24.8125 95.6849 49.4975 71 79.95 71C110.402 71 135.087 95.6849 135.087 126.137C135.087 156.59 110.402 181.275 79.95 181.275C49.4975 181.275 24.8125 156.59 24.8125 126.137ZM36.8204 126.16C36.8204 149.97 56.1238 169.273 79.9339 169.273C103.744 169.273 123.047 149.97 123.047 126.16C123.047 102.35 103.744 83.0465 79.9339 83.0465C56.1238 83.0465 36.8204 102.35 36.8204 126.16Z"
              fill="white"
            />
            <path
              d="M155.967 126.141C155.967 95.7043 180.642 71.029 211.079 71.029C241.515 71.029 266.187 95.7043 266.187 126.141C266.187 156.577 241.512 181.249 211.076 181.249C180.639 181.249 155.964 156.574 155.964 126.137L155.967 126.141ZM168.004 126.179C168.004 149.989 187.307 169.293 211.117 169.293C234.927 169.293 254.231 149.989 254.231 126.179C254.231 102.369 234.927 83.0658 211.117 83.0658C187.307 83.0658 168.004 102.369 168.004 126.179Z"
              fill="white"
            />
            <path d="M129 123H161" stroke="white" strokeWidth="10" />
          </g>
          <defs>
            <clipPath id="clip0_8_119">
              <rect width="241.375" height="147" fill="white" transform="translate(266.187 218) rotate(-180)" />
            </clipPath>
          </defs>
        </svg>
        <span className="font-bold text-xl ml-2 text-[#0066CC]">ScoutFiles</span>
      </div>
    </Link>
  )
}

